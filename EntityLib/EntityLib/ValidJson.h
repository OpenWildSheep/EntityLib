...
namespace Ent
{
    ...
    void validScene(Schema const& schema, std::filesystem::path toolsDir, nlohmann::json const& scene);
    /// Valid an Entity using the JSON scheme of entities
    /// @throw JsonValidation when the Entity in invalid
    void validEntity(Schema const& schema, std::filesystem::path toolsDir, nlohmann::json const& entity);
} // namespace Ent
