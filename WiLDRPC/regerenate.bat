"../../External/FlatBuffers/Release/flatc" --cpp Float.fbs
"../../External/FlatBuffers/Release/flatc" --python Float.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Integer.fbs
"../../External/FlatBuffers/Release/flatc" --python Integer.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Boolean.fbs
"../../External/FlatBuffers/Release/flatc" --python Boolean.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Vector3.fbs
"../../External/FlatBuffers/Release/flatc" --python Vector3.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Vector2.fbs
"../../External/FlatBuffers/Release/flatc" --python Vector2.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Quat.fbs
"../../External/FlatBuffers/Release/flatc" --python Quat.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Color.fbs
"../../External/FlatBuffers/Release/flatc" --python Color.fbs

"../../External/FlatBuffers/Release/flatc" --cpp UInt3.fbs
"../../External/FlatBuffers/Release/flatc" --python UInt3.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Camera.fbs
"../../External/FlatBuffers/Release/flatc" --python Camera.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Result.fbs
"../../External/FlatBuffers/Release/flatc" --python Result.fbs

"../../External/FlatBuffers/Release/flatc" --cpp Position.fbs
"../../External/FlatBuffers/Release/flatc" --python Position.fbs

"../../External/FlatBuffers/Release/flatc" --cpp String.fbs
"../../External/FlatBuffers/Release/flatc" --python String.fbs

"../../External/FlatBuffers/Release/flatc" --cpp RPCHeader.fbs
"../../External/FlatBuffers/Release/flatc" --python RPCHeader.fbs
pause