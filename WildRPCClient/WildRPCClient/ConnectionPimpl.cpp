...
namespace WRPC
{
    ...
    ConnectionStatus ConnectionPimpl::Open()
    {
        try
        {
            ...
            asio::connect(
                *m_socket_NOT_ThreadSafe,
                resolver.resolve(m_IPaddress.c_str(), SERV_PORT_NOT_THREAD_SAFE));

            m_socket_ThreadSafe = new asio::ip::tcp::socket(m_io_context);
            asio::connect(
                *m_socket_ThreadSafe, resolver.resolve(m_IPaddress.c_str(), SERV_PORT_THREAD_SAFE));
        }
